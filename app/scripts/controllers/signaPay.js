...
angular.module('feApp')
  .controller('AboutCtrl', function ($scope,Events) {
      ...
      Events.getRemainingBudget().then(data=>{
          $scope.remainBudget = data;
      });


      Events.budgetChartMaster().then(data=>{
          console.log(data);
          Highcharts.chart('dailySalesChart',{
              ...
              tooltip: {
                  pointFormat: '{series.data.name}<b>{point.percentage:.2f}%</b>',
                  borderWidth: 2,
                  borderRadius: 5
              },
              plotOptions: {
      ...
      Events.budgetChartCurrent().then(data=>{
          console.log(data);

          data.map((gdata,ind)=>{
              Highcharts.chart('completedTasksChart'+(ind+1),{
                  chart: {
                      plotBackgroundColor: 'rgba(255, 255, 255, 0)',
                      backgroundColor : 'rgba(255, 255, 255, 0)',
                      plotBorderWidth: null,
